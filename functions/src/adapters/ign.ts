...
export async function fetchIgnNews(): Promise<NewsDocument[]> {
  const items = await fetchRSSFeed(CONFIG.FEEDS.IGN, 'IGN BR');
  const results: NewsDocument[] = [];

  for (const item of items.slice(0, 10)) {
    const rawTitle = item.title || 'Notícia IGN';
    ...
    results.push({
      ...
      url: rawUrl,
      source: 'IGN Brasil',
      author: item.creator || item.author || 'IGN Staff',
      publishedAt: pubDate,
