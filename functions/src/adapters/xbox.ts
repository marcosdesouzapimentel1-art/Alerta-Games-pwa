...
export async function fetchXboxNews(): Promise<NewsDocument[]> {
  const items = await fetchRSSFeed(CONFIG.FEEDS.XBOX, 'Xbox Wire');
  const results: NewsDocument[] = [];

  for (const item of items.slice(0, 10)) {
    const rawTitle = item.title || 'Xbox Wire';
    ...
    results.push({
      ...
      url: rawUrl,
      source: 'Xbox Wire',
      author: item.creator || item.author || 'Xbox Wire Staff',
      publishedAt: pubDate,
      category: 'Xbox',
      tags: ai.tags,
      ...
      views: Math.floor(Math.random() * 380) + 140,
      likes: Math.floor(Math.random() * 75) + 18,
      language: 'pt-BR',
      status: 'published'
    });
  }
