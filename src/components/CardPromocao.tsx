...
export const CardPromocao: React.FC<CardPromocaoProps> = ({ deal }) => {
  ...
  const handleCopyCoupon = (e: React.MouseEvent) => {
    ...
    if (deal.couponCode) {
      ...
      showToast(`Cupom ${deal.couponCode} copiado!`);
    ...
    >
      <div>
        ...
        <div className="p-4 sm:p-5">
          <div className="flex items-center justify-between gap-2 mb-2">
            <span className={`px-2.5 py-0.5 rounded-md text-[11px] font-bold border ${getStoreBadgeColor(deal.store)}`}>
              {deal.store}
            </span>
          ...
          {deal.couponCode && (
            ...
            >
              <div className="flex items-center gap-1.5 text-slate-300 font-mono">
                ...
                <span className="font-bold text-emerald-400">{deal.couponCode}</span>
              </div>
      ...
      <div className="px-4 sm:px-5 pb-4 pt-2 border-t border-slate-800/60 flex items-center justify-between text-xs text-slate-400">
        <span className="text-[11px] text-slate-400">
          Validade: {new Date(deal.expiresAt).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}
        </span>
