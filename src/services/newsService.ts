      ...
      if (response.ok) {
        ...
        if (Array.isArray(data.articles)) {
          return data.articles.map((art: any, index: number) => ({
            ...
            title: art.titlePt || art.title || 'Sem título',
            summary: art.summaryPt || art.description || art.summary || 'Resumo indisponível.',
            content: art.summaryPt || art.content || art.description || '',
...
export const getNewsFromFirestore = async ({
  ...
  try {
    ...
    let articles: NewsArticle[] = docs.map((docSnap) => {
      ...
      return {
        id: docSnap.id,
        // Prioriza titlePt (português) caso exista, senão faz fallback para title
        title: data.titlePt || data.title || 'Sem título',
        // Prioriza summaryPt (português) caso exista, senão faz fallback para summary/content
        summary: data.summaryPt || data.summary || data.content || '',
        ...
        content: data.summaryPt || data.content || data.summary || '',
